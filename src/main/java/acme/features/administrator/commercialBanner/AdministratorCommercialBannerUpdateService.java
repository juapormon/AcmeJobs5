...
public class AdministratorCommercialBannerUpdateService implements AbstractUpdateService<Administrator, CommercialBanner> {
	...
	public void validate(final Request<CommercialBanner> request, final CommercialBanner entity, final Errors errors) {
		...
		if (!errors.hasErrors("creditCardMonth") && !errors.hasErrors("creditCardYear")) {
			Calendar currentCalendar = new GregorianCalendar();
			Calendar expiration = new GregorianCalendar(entity.getCreditCardYear(), entity.getCreditCardMonth(), 1);
			boolean isFuture = expiration.getTime().after(currentCalendar.getTime());
			errors.state(request, isFuture, "creditCardYear", "administrator.commercial-banner.form.error.credit-card-expirated");
		}
	}
